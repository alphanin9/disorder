...
def _toml_quote(value: str) -> str:
    # TOML basic string escaping is close to JSON string escaping for our use-case.
    return json.dumps(value)


def _toml_array(values: list[str]) -> str:
    return "[" + ", ".join(_toml_quote(item) for item in values) + "]"


def _write_codex_config(spec: dict[str, Any]) -> None:
    """
    Codex discovers MCP servers via ${CODEX_HOME}/config.toml.

    Policy:
    - Deterministic: generated every run, overwriting any existing config.toml.
    - No merge with host config.
    """
    codex_home = Path(os.getenv("CODEX_HOME") or (Path.home() / ".codex"))
    codex_home.mkdir(parents=True, exist_ok=True)
    config_path = codex_home / "config.toml"

    servers: list[tuple[str, str, list[str], int]] = []
    ...
    if _env_truthy("CODEX_FLAG_VERIFY_MCP_ENABLED", default=True):
        servers.append(
            (
                "flag_verify",
                "python",
                ["/usr/local/bin/flag_verify_mcp.py", "--spec", str(SPEC_PATH)],
                30,
            )
        )

    if _env_truthy("CODEX_CRYPTO_MCP_ENABLED", default=True):
        servers.append(
            (
                "crypto_math",
                "python",
                ["/usr/local/bin/crypto_math_mcp.py", "--workspace", str(RUN_DIR)],
                60,
            )
        )

    if _env_truthy("CODEX_GHIDRA_MCP_ENABLED", default=True):
        servers.append(
            (
                "ghidra",
                "python",
                ["/usr/local/bin/ghidra_mcp.py", "--workspace", str(RUN_DIR)],
                600,
            )
        )

    ida_enabled = _env_truthy("CODEX_IDA_MCP_ENABLED", default=False) or _env_truthy("SANDBOX_IDA_MCP_ENABLED", default=False)
    if ida_enabled and Path("/opt/ida").exists():
        servers.append(
            (
                "ida",
                "python",
                ["/usr/local/bin/ida_mcp.py", "--ida-dir", "/opt/ida", "--workspace", str(RUN_DIR)],
                30,
            )
        )

    lines: list[str] = []
    lines.append("# Auto-generated by ctf-agent-sandbox agent_runner.py. DO NOT EDIT.\n")
    for name, command, args, timeout_sec in servers:
        lines.append(f"[mcp_servers.{name}]")
        lines.append(f"command = {_toml_quote(command)}")
        lines.append(f"args = {_toml_array(args)}")
        lines.append(f"startup_timeout_sec = {int(timeout_sec)}")
        lines.append("")
