...
FINAL_RUN_STATUSES = {"flag_found", "deliverable_produced", "blocked", "timeout"}
...
def _has_active_runs_for_challenge(db: Session, challenge_id: UUID) -> bool:
    stmt = select(Run.id).where(Run.challenge_id == challenge_id, Run.status.not_in(FINAL_RUN_STATUSES)).limit(1)
