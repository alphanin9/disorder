...
def sync_ctfd(request: CTFdSyncRequest, db: Session = Depends(get_db)) -> CTFdSyncResponse:
    try:
        return CTFdSyncResponse.model_validate(sync_ctfd_challenges(db, request))
