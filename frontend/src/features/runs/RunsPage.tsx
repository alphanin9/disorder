  ...
  return (
    <div className="space-y-4">
      <Card>
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-ink-muted">Queued and running sandboxes refresh automatically.</p>
          </div>
        ...
        {activeRunsQuery.data && activeRunsQuery.data.items.length === 0 ? <p className="text-sm text-ink-muted">No active runs right now.</p> : null}
            ...
            <table className="min-w-full border-collapse text-sm">
              ...
              <tbody>
                {activeRunsQuery.data.items.map((run) => (
                  <tr key={run.id} className="border-b border-line/60 last:border-b-0">
                    ...
                    <td className="px-2 py-3 text-ink-muted">{formatDateTime(run.started_at)}</td>
      ...
      <Card>
        ...
        {recentRunsQuery.data && recentRunsQuery.data.items.length === 0 ? <p className="text-sm text-ink-muted">No runs yet.</p> : null}

        {recentRunsQuery.data && recentRunsQuery.data.items.length > 0 ? (
          <ul className="space-y-2 text-sm">
            {recentRunsQuery.data.items.map((run) => (
              <li key={run.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md bg-surface-muted px-3 py-2">
                <div className="flex items-center gap-2">
                  <Link className="font-semibold text-accent hover:underline" to={`/runs/${run.id}`}>
                    {run.id.slice(0, 8)}
                  </Link>
                  <span className="text-ink-muted">{run.backend}</span>
                ...
                <div className="flex items-center gap-2">
                  <Badge status={run.status}>{run.status}</Badge>
                  <span className="text-xs text-ink-muted">{formatDateTime(run.started_at)}</span>
